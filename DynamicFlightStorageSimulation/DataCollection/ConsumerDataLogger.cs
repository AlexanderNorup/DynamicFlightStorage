...
namespace DynamicFlightStorageSimulation.DataCollection
{
    public class ConsumerDataLogger
    {
        ...
        private readonly MessagePackSerializerOptions _msgPackOptions = MessagePackSerializerOptions.Standard.WithCompression(MessagePackCompression.Lz4BlockArray);
        ...
        public async Task PersistDataAsync(string experimentId)
        {
            ...
            var flightData = MessagePackSerializer.Serialize(_flightLog, _msgPackOptions);
            var weatherData = MessagePackSerializer.Serialize(_weatherLog, _msgPackOptions);
        ...
        public async Task<List<WeatherLog>?> GetWeatherLog(int dbId)
        {
            if (await _dbContext.WeatherEventLogs.FirstOrDefaultAsync(x => x.Id == dbId) is { } weatherData)
            {
                return MessagePackSerializer.Deserialize<List<WeatherLog>>(weatherData.WeatherData, _msgPackOptions);
            }
            return null;
        }

        public async Task<List<FlightLog>?> GetFlightLog(int dbId)
        {
            if (await _dbContext.FlightEventLogs.FirstOrDefaultAsync(x => x.Id == dbId) is { } flightData)
            {
                return MessagePackSerializer.Deserialize<List<FlightLog>>(flightData.FlightData, _msgPackOptions);
            }
            return null;
        }
